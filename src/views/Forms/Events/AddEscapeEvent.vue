...
const goback = async () => {
  await router.push('/event/escape/list')
}

const execute = async () => {
  if(!valid) return;
  let res = await makeAddEscapeEventGetRequest({
...
<template>
  <div class="add-form-animation flex flex-col justify-center items-center w-full gap-2">
    <h1 class="text-secondary text-xl text-center font-bold my-5">Add Escape Event</h1>
