...
import QueryBuffer, {AlcoholicEntityPickerMulti, DrinkEntityPicker, nValidator, Query} from "@/data/QueryBuffer";
...
const input = ref<Query>({
  ...
  input: (new InputGenerator())
      .addNumber('event_id', 'Join Event ID')
      .add(AlcoholicEntityPicker)
      .add(DrinkEntityPicker)
      .finish(),
  url: ''
});
...
<template>
  <div class="add-form-animation flex flex-col justify-center items-center w-full gap-2">
    <h1 class="text-secondary text-xl text-center font-bold my-5">Add Bed Change Event</h1>
